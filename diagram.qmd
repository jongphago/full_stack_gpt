...
sequenceDiagram
    ...
    participant RunnableLambda
    participant ChatPromptTemplate
    participant ChatOpenAI
    participant Runnable

    par Runnable.invoke
        Note right of Query: message: str
        Query -->>+ Runnable: message: str
        Note right of Runnable: chain: Runnable
        Runnable ->> Runnable: invoke
        Runnable ->>+ RunnablePassthrough: message: str
        Runnable ->>+ VectorStoreRetriever: message: str
        Note right of VectorStoreRetriever: retriever: VectorStoreRetriever
    end
    par Prompt
        VectorStoreRetriever ->>- RunnableLambda: List[str]
        activate RunnableLambda
        RunnableLambda ->>- ChatPromptTemplate: context
        activate ChatPromptTemplate
        RunnablePassthrough ->>- ChatPromptTemplate: question
        Note right of ChatPromptTemplate: prompt: ChatPromptTemplate
    end
    ChatPromptTemplate ->>- ChatOpenAI: PromptValue
    activate ChatOpenAI
    Note right of ChatOpenAI: llm: ChatOpenAI
    ChatOpenAI ->>- Runnable: response: str
    deactivate Runnable
```